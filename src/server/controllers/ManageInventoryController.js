...
const ManageInventoryController = {
  ...
  async updateInventory(newInventoryData, action) {
    ...
    if (action === 'create') {
      ...
      url =
        `${inventoryApiConfig.deleteInventoryEndPoint}` +
        newInventoryData.inventoryId;
    ...
    const newStock =
      parseInt(
        newInventoryData.inventoryAvlStock
          ? newInventoryData.inventoryAvlStock
          : 0
      ) +
      parseInt(
        newInventoryData.inventoryAddStock
          ? newInventoryData.inventoryAddStock
          : 0
      );
    ...
    try {
      ...
      console.log(
        `error in creating/editing an inventory with inventoryName ${newInventoryData.inventoryName} : ` +
          error
      );
    }
